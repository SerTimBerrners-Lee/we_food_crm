...
export default defineComponent({
  ...
  setup(props) {
    ...
    const showDrawer = async () => {
			...
			form.max_date_actions = ref(dayjs(new Date(promo.max_date_actions).toLocaleDateString('ru-RU'), dateFormat));
    ...
    const onSave = async (formaingData) => {
      state.loading = true;
      const result = await store.dispatch('updatePromo', formaingData);
      state.loading = false;
