  ...
  <a-table :columns="columns" :data-source="getOrders" @change="onChange">
    ...
    <template #bodyCell="{ column, record }">
      ...
      <template v-else-if="column.key === 'action'">
        <span>
          <edit-order :order_id="record.id" @unmount="unmount_edit_order" />
...
export default defineComponent({
  ...
  setup() {
    ...
    store.dispatch('getNotConfirmedOrders', { page: 1, limit: 25 }).then(res => {
      if (!res.success)
    ...
    const precentStage = (order) => {
      ...
      return precent;
		...
		const unmount_edit_order = (order) => {
			if (!order) return false;
			const { id, processing } = order;
			if (processing === 'confirmed') store.commit("delete_order", id);
		}
