<template>
	...
	<div style="background: #ececec; padding: 30px" class="container_instruction">
		<a-card v-for="stage of getStages" :key="stage.id" :title="'#' + stage.id" :bordered="false" style="width: 40%">
			<template #extra>
				<a-button type="primary" v-if="!loading" @click="toComplited(stage.id)">Подтвердить доставку</a-button>
				<LoaderSpin v-else />
			</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const reloadStages = async () => {
			const res = await store.dispatch('getStages', { page: 1, limit: 25, status: 1 });
			if (!res.success)
      ...
      if (!id) return message.warn("Не правильно переданы параметры для удаления");
      const result = await store.dispatch("", id);
      if (result.success) 
    ...
    const formatDateRange = (data) => {
      ...
      return `C <strong>${splt[0]}</strong> по <strong>${splt[1]}</strong>`;
		...
		const toComplited = async (id) => {
			state.loading = true;
			const result = await store.dispatch("fetchToComplited", id);
			state.loading = false;

			if (!result.success) return message.error(result.error || 'Произошла ошибка');
			return message.success("Доставлено");
		}
